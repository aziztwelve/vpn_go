...
**Статус реализации:** ✅ **реализовано** (Этап 3 MVP, 2026-04-22)
...
| Файл | Что делает |
|---|---|
| [`services/vpn-service/internal/service/vpn.go`](../../services/vpn-service/internal/service/vpn.go) | `GenerateVLESSLink(userID, serverID, deviceID)` — проверка `ErrDeviceLimitExceeded`, UPSERT `active_connections`, возврат `VLESSLinkResult{Link, ConnectionID, CurrentDevices, MaxDevices}`. Константа `DeviceActivityWindow = 5 * time.Minute`. |
| [`services/vpn-service/internal/service/heartbeat.go`](../../services/vpn-service/internal/service/heartbeat.go) | `Heartbeat{prevSeen}` + `Run(ctx)` — тикер 60с, опрос `xray.GetUserStats`, `UpdateLastSeenByVPNUser` при росте трафика |
| [`services/vpn-service/internal/app/app.go`](../../services/vpn-service/internal/app/app.go) | `a.heartbeat = service.NewHeartbeat(...)` + `go a.heartbeat.Run(hbCtx)` в `Start()`, остановка через `closer` |
| [`services/vpn-service/internal/repository/vpn.go`](../../services/vpn-service/internal/repository/vpn.go) | `CountActiveDevices(userID, window)`, `UpsertActiveConnection(...)`, `UpdateLastSeenByVPNUser(...)`, `GetSubscriptionMaxDevices(vpnUserID)`, `ListAllVPNUsers()` |
| [`services/vpn-service/internal/api/vpn.go`](../../services/vpn-service/internal/api/vpn.go) | gRPC `GetVLESSLink` ловит `ErrDeviceLimitExceeded` → `codes.ResourceExhausted` |
| [`shared/proto/vpn/v1/vpn.proto`](../../shared/proto/vpn/v1/vpn.proto) | `GetVLESSLinkRequest.device_identifier` + `Response.{current_devices, max_devices, connection_id}` |
| [`services/gateway/internal/handler/vpn.go`](../../services/gateway/internal/handler/vpn.go) | `GET /api/v1/vpn/servers/:serverId/link?device_id=X&user_id=N` — 429 при ResourceExhausted, `DELETE /api/v1/vpn/devices/:connectionId` |
| [`services/gateway/internal/app/app.go`](../../services/gateway/internal/app/app.go) | Роут `r.Delete("/vpn/devices/{connectionId}", ...)` |
