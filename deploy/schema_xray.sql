...
COMMENT ON TABLE xray_servers IS 'Xray VLESS серверы в разных локациях';
COMMENT ON COLUMN xray_servers.public_key IS 'Reality public key для клиентов';
COMMENT ON COLUMN xray_servers.private_key IS 'Reality private key для сервера';
COMMENT ON COLUMN xray_servers.short_id IS 'Reality short ID';
COMMENT ON COLUMN xray_servers.dest IS 'Reality destination (маскировка)';
COMMENT ON COLUMN xray_servers.server_names IS 'SNI для Reality';
COMMENT ON COLUMN xray_servers.api_host IS 'Xray API host (обычно 127.0.0.1)';
COMMENT ON COLUMN xray_servers.api_port IS 'Xray API port (обычно 10085)';

CREATE INDEX idx_xray_servers_active ON xray_servers(is_active);
CREATE INDEX idx_xray_servers_country ON xray_servers(country_code);
...
-- 6. Xray пользователи (один UUID на все серверы)
...
COMMENT ON TABLE xray_users IS 'Xray пользователи с UUID (один UUID на все серверы)';
COMMENT ON COLUMN xray_users.uuid IS 'UUID для подключения к Xray (один на все серверы)';
COMMENT ON COLUMN xray_users.email IS 'Email идентификатор в Xray (формат: user_{user_id}_{subscription_id})';
COMMENT ON COLUMN xray_users.flow IS 'Xray flow (xtls-rprx-vision)';

CREATE INDEX idx_xray_users_user_id ON xray_users(user_id);
CREATE INDEX idx_xray_users_subscription_id ON xray_users(subscription_id);
CREATE INDEX idx_xray_users_uuid ON xray_users(uuid);

-- 7. Активные подключения (для контроля лимита устройств)
CREATE TABLE active_connections (
    id BIGSERIAL PRIMARY KEY,
    xray_user_id BIGINT NOT NULL REFERENCES xray_users(id) ON DELETE CASCADE,
    server_id INT NOT NULL REFERENCES xray_servers(id),
    device_identifier VARCHAR(255), -- IP или другой идентификатор устройства
    connected_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    last_seen TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE(xray_user_id, server_id, device_identifier)
);
...
CREATE INDEX idx_active_connections_xray_user_id ON active_connections(xray_user_id);
...
CREATE INDEX idx_traffic_logs_xray_user_id ON traffic_logs(xray_user_id);
